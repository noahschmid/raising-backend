...
public class SettingRepository {
	...
	public SettingRepository(JdbcTemplate jdbc) {
		this.jdbc = jdbc;
		this.INSERT_DEVICE_TOKEN = "INSERT INTO settings(accountid, token, device, notificationtypes) VALUES (?,?,?,?)";
		this.SELECT_BY_ACCOUNTID = "SELECT * FROM settings WHERE accountid = ?;";
	}
	...
	public void addSettings(Settings settings) throws DataAccessException, SQLException{
		...
		jdbc.update(INSERT_DEVICE_TOKEN, new Object[] {settings.getAccountId(), settings.getToken(), device, notificationTypes }, new int[] {Types.BIGINT, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR});
	}
